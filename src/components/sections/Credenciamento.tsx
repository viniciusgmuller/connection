...
const beneficiosCredencial = [
  {
    icon: 'content',
    title: 'Conteúdo exclusivo',
    description: 'Acesso a todas as palestras, painéis e debates com especialistas.',
  },
  {
    icon: 'business',
    title: 'Rodadas de negócios',
    description: 'Participe de encontros B2B e conecte-se com produtores e compradores.',
  },
  {
    icon: 'gastro',
    title: 'Experiências gastronômicas',
    description: 'Degustações exclusivas, aulas show e harmonizações com chefs renomados.',
  },
  {
    icon: 'opening',
    title: 'Coquetel de abertura',
    description: 'Jantar exclusivo de abertura com networking e produtos de origem selecionados.',
  },
];
  ...
  return (
    <section className="relative bg-[#131415] py-16 md:py-24">
      <div className="mx-auto max-w-[1330px] px-4 md:px-8">
        ...
        <div
          ref={headerRef}
          className={`mb-12 flex flex-col gap-10 lg:flex-row lg:items-start lg:justify-between md:mb-16 scroll-animate ${headerVisible ? 'is-visible' : ''}`}
        >
        ...
        <div
          ref={gridRef}
          className={`mb-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 md:gap-5 scroll-stagger ${gridVisible ? 'is-visible' : ''}`}
        >
          {beneficiosCredencial.map((b, i) => (
            <div
              key={i}
        ...
        <div
          ref={ctaRef}
          className={`flex flex-col items-center justify-center gap-4 sm:flex-row scroll-animate ${ctaVisible ? 'is-visible' : ''}`}
        >
          <Link
            href="/ingressos"
            className="inline-flex w-[280px] items-center justify-center rounded-full bg-[#C9A962] px-8 py-4 font-just-sans text-base font-semibold text-[#131415] transition-colors hover:bg-[#C9A962]/90"
          >
            Garantir minha credencial
          </Link>
          <Link
            href="#sobre"
            className="inline-flex w-[280px] items-center justify-center rounded-full border border-[#FFF5EC]/20 px-8 py-4 font-just-sans text-base font-semibold text-[#FFF5EC] transition-colors hover:bg-[#FFF5EC]/5"
          >
            O que está incluso?
          </Link>
        </div>
